...
var tm = window.tm || {
        ...
        debug : function(){
            ...
            window.setInterval(function(){
                $('#info').html(self.which_segment + ' - '+ self.point_selected);
                /*
                 if(typeof self.o.points != 'undefined'){
                 //var $o = self.o.points;
                 var h = '';
                 var p = '';
                 var po;
                 ...
                 $.each(self.o.segments, function(i,v){
                 if(v == null){
                 po = 'null';
                 } else {
                 po = $.dump(this.points);
                 ...
                 // sort numbers ascending to easily test
                 $.each(self.o.points, function(){
                 if(this > -1){
                 if(this.i.sort){
                 this.i.sort(function(a,b){ return a - b });
                 }
                 }
                 });
                 $.each(self.o.points, function(i,v){
                 if(this.i){
                 po = $.dump(this.i.sort(function(a,b){return a-b}));
                 } else {
                 po = $.dump(this.i);
                 }
                 p += i+': '+po+'<br />';
                 ...
                 $.each(self.o.segments, function(si,sv){
                 if(this.points){
                 $.each(this.points, function(pi,pv){

                 // FIX - missing spaces are undefined
                 a[pv] = a[pv] || [];
                 a[pv].push(si);
                 });
                 }
                 });
                 $.each(a, function(ii,vv){
                 po2 = $.dump(a[ii]);
                 p2 += ii+': '+po2+'<br />';
                 });

                 var are_they_equal = window.compareAssociativeArrays(p,p2);
                 var all = are_they_equal ? '<br /><br />Points:<br />'+p : '<br /><br />Points:<br />'+p+'<br />Points2:<br />'+p2;

                 var misc = '<br /><br />' + $('#g_canvas').css('top');

                 var seg_selected = self.segment_selected != null ? ', Segment selected: <strong>'+self.segment_selected+'</strong>' : '';

                 benchmark = self.endTime - self.startTime;

                 $('#info').html('Benchmark (redraw screen time): '+benchmark+'<br />Equal: '+are_they_equal+seg_selected+'<br /><br />Segments:<br />'+h+all+misc);
                 }*/
            }, 2000);
        },
        ...
        init : function(){
            ...
            //        if ((e.which && e.which == 13) || (e.keyCode && e.keyCode == 13)) {
            //        	if(!self.o.territories[$('#left_input #left').val()]){
            //             $('#territories #list').append('<p>' + $('#left_input #left').val() + '</p>');
            //             $('#left_input').css('display','none');
            //             self.saveTerritory('left', $('#left_input #left').val());
            //             self.redrawSegments({highlight: { right: self.highlight.right, left: null }});
            //         }
            //         return false;
            //        }
            //    });
            // $('#right_input #right').bind('keypress', function(e) {
            //        if ((e.which && e.which == 13) || (e.keyCode && e.keyCode == 13)) {
            //        	if(!self.o.territories[$('#right_input #right').val()]){
            //             $('#territories #list').append('<p>' + $('#right_input #right').val() + '</p>');
            //             $('#right_input').css('display','none');
            //             self.saveTerritory('right', $('#right_input #right').val());
            //             self.redrawSegments({highlight: { left: self.highlight.left, right: null }});
            //         }
            //         return false;
            //        }
            //    });
        ...
        getTerritories : function(){
            ...
            if(!self.all_territory_outlines){
                try {
                    $.ajax({
                        url: '../get_territories.php',
                        dataType: 'JSON',
                        success: function(data){
                            self.all_territory_outlines = data;
                        }
                    });
        ...
        getOutTerritories : function(){
            var self = this;

            try {
                $.ajax({
                    url: '../get_out_territories.php',
                    dataType: 'JSON',
                    success: function(data){
                        self.out_territories = data;
                        self.getPointsNotToMove();
                    }
                });
        ...
        plotGeocodes : function(){
            ...
            if((self.geocodes != null) && (!self.geocodes_hidden)){
                ...
                var factor = self.o.preferences.zoom < 21 ? Math.pow(2, Math.floor((20 - self.o.preferences.zoom) / 1.75)) : 1;

                $.each(self.geocodes, function(k,v){

                    if(v[0] < self.o.segment_bounds.y1){
                        var point_size = Math.floor(v[2] / factor) + 1,
                            point_2 = v[2] >= 4 ? true : false;
                        ...
                        c.fillRect(p_x - point_size, p_y - point_size, point_size * 2, point_size * 2);
                        if(point_2){
                            c.fillRect(p_x - point_size / 2, p_y - point_size / 2, point_size, point_size);
                        }
                    }

                    // break out of loop if outside of bounds
                    if(v[0] < self.o.segment_bounds.y2){
                        return;
                    }
                });
        ...
        removeTerritoryFromRecords : function(t, cb){
            try {
                $.ajax({
                    type : "GET",
                    url : "../remove-territory-from-records.php?t=" + t,
                    dataType : 'json',
                    cache : false,
        ...
        createTodaysGeocodes : function(days){
            try {
                $.ajax({
                    type : "GET",
                    url : "../create-geocodes.php?days=" + days,
                    dataType : 'json',
                    cache : false,
                    success : function(html){
                        loadJSFile("cache/geocodes-" + days + ".js");
                    }
                });
            } catch(e){}
        },
        ...
        // 33.901846,-84.442287;33.907811,-84.447372;33.910506,-84.447437;33.912078,-84.446532;33.916722,-84.439938;33.918734,-84.438426;33.922121,-84.43744;33.923334,-84.43563;33.924079,-84.432152;33.925409,-84.429429;33.927035,-84.427227;33.931805,-84.418164;33.935883,-84.413623;33.938012,-84.411872;33.945172,-84.403484;33.951451,-84.397803;33.955682,-84.390884;33.956975,-84.389545;33.955053,-84.385671;33.955745,-84.382311;33.955969,-84.376725;33.955394,-84.374098;33.955224,-84.370952;33.955511,-84.369623;33.955835,-84.366442;33.955331,-84.36457;33.95455,-84.364989;33.953095,-84.366629;33.95172,-84.367687;33.950903,-84.368226;33.949205,-84.370247;33.948334,-84.370615;33.946223,-84.371058;33.943689,-84.372139;33.94245,-84.373106;33.937536,-84.377963;33.937015,-84.378307;33.935164,-84.379492;33.934006,-84.379882;33.932721,-84.380061;33.931508,-84.379907;33.929442,-84.37917;33.925202,-84.37907;33.922939,-84.379141;33.919857,-84.378951;33.915572,-84.379307;33.913192,-84.379185;33.912231,-84.378998;33.912321,-84.384911;33.91392,-84.39753;33.915662,-84.402656;33.916004,-84.405675;33.915806,-84.41695;33.914557,-84.421565;33.912734,-84.424737;33.905386,-84.433289;

        // save to local JS
        saveTerritory : function(left_or_right, territory_name){
            var self = this;

            self.o.territories[territory_name] = self.selected_polygon.segments;

            self.saveTerritoryToDatabase(territory_name, self.selected_polygon.points.slice(0), territory_name);

            return false;
        },

        // saves to database through Ajax
        saveTerritoryToDatabase : function(territory, points, specific_territory){
            var self = this,
                polygon = '';

            specific_territory = specific_territory || '';

            if(points){
                $.each(points, function(k, v){
                    var p = self.o.points[v];
                ...
                $.ajax({
                    type: "POST",
                    url: '../save-territory.php',
                    data: data,
                    success: function(data) {
                        $('#points').html('<h3>Saved ' + data + '</h3>');
                        $.ajax({
                            type: "GET",
                            url: '../file-orphan-records.php?t=' + specific_territory,
                            success: function(data) {
                                console.log('Orphan records filed');
                            }
                        });
                    },
                    dataType: 'text'
                ...
                // $.post('save-territory.php', function(data) {
                // 	$('#points').html('<h3>Saved ' + data + '</h3>');
                // });
            }
        },
        ...
        drawPolygonOverlays : function(t){
            var self = this,
                c = [self.l_c[8], self.l_c[9], self.l_c[10], self.l_c[11], self.l_c[12], self.l_c[13]];

            var population_fill_style = ['rgba(255,255,255,0.5)', 'rgba(204,204,255,0.3)', 'rgba(153,153,255,0.4)','rgba(204,51,255,0.4)','rgba(255,0,204,0.4)','rgba(255,0,51,0.5)'];
            var territories_fill_style = ['rgba(255,51,255,0.5)', 'rgba(255,255,51,0.5)', 'rgba(51,255,255,0.5)','rgba(51,51,255,0.5)','rgba(255,51,51,0.5)','rgba(51,0,51,0.5)'];
            var status_fill_style = ['rgba(0,0,255,0.1)', 'rgba(255,204,51,0.1)', 'rgba(255,51,51,0.1)'];
            var out_fill_style = 'rgba(255,51,51,0.4)';
            var geocode_clone = $.extend(true, [], self.geocodes),
                geocode_length = geocode_clone.length - 1;
            ...
            if(!self.territory_outlines){
                self.territory_outlines = {};
            }

            $.each(c, function(k, v){
                c[k].clearRect(0, 0, self.extended_canvas_width, self.extended_canvas_height);
            });

            $("#population_legend").html('');

            $.each(self.o.territories, function(k, v){
                var x, count = 0;

                if(!self.territory_outlines[k]){
                    self.territory_outlines[k] = {};
                }
                if(!self.territory_outlines[k].outline){
                    self.territory_outlines[k].outline = [];
                }
                if(!self.territory_outlines[k].count){
                    self.territory_outlines[k].count = 0;
                }

                if(!self.territory_outlines[k].outline.length){
                    self.territory_outlines[k].outline = self.normalizeSegments(v);
                }

                if(t === 'population'){
                    if(!self.territory_outlines[k].count){
                        for(x = geocode_length; x >= 0; x--){
                            if(self.isInside(self.territory_outlines[k].outline, geocode_clone[x][0], geocode_clone[x][1])){
                                count += geocode_clone[x][2];
                                geocode_clone.splice([x], 1);
                            }
                        }
                        self.territory_outlines[k].count = count;
                        geocode_length = geocode_clone.length - 1;
                    }
                }
            });
            ...
            if(t === 'population'){
                // CLEAN UP LATER
                var range = [[],[],[],[],[],[]],
                    lowest = 10000, highest = 0, difference = 0,
                    total = self.territory_outlines.length - 1,
                    per_each;

                $.each(self.territory_outlines, function(k, v){
                    if(lowest > parseInt(v.count)){
                        lowest = parseInt(v.count);
                    }
                    if(highest < parseInt(v.count)){
                        highest = parseInt(v.count);
                    }
                });
                difference = highest - lowest;
                per_each = Math.floor(difference / 6);
                range[0][0] = lowest;
                range[0][1] = lowest + per_each;
                range[1][0] = lowest + per_each + 1;
                range[1][1] = lowest + (per_each * 2);
                range[2][0] = lowest + (per_each * 2) + 1;
                range[2][1] = lowest + (per_each * 3);
                range[3][0] = lowest + (per_each * 3) + 1;
                range[3][1] = lowest + (per_each * 4);
                range[4][0] = lowest + (per_each * 4) + 1;
                range[4][1] = lowest + (per_each * 5);
                range[5][0] = lowest + (per_each * 5) + 1;
                range[5][1] = highest;

                $.each(c, function(k, v){
                    c[k].fillStyle = population_fill_style[k];
                    c[k].clearRect(0, 0, self.extended_canvas_width, self.extended_canvas_height);
                    c[k].beginPath();
                    $("#population_legend").append('<p style="background: ' + population_fill_style[k] + ';">' + range[k][0] + ' to ' + range[k][1] + '</p>');
                });
            } else if(t === 'territories'){
                if(self.all_territory_outlines){
                    c[0].fillStyle = out_fill_style;
                    c[0].clearRect(0, 0, self.extended_canvas_width, self.extended_canvas_height);
                    c[0].beginPath();

                    var a = self.all_territory_outlines.slice(0);
                    $.each(self.territory_outlines, function(k, v){
                        if($.inArray(k, a) > -1){
                            // go through each point
                            $.each(v.outline, function(points_index, points_value){
                                // first is moveto
                                if(points_index === 0){
                                    c[0].moveTo(self.o.points[points_value].x, self.o.points[points_value].y);
                                    // all others are lineto
                                } else {
                                    c[0].lineTo(self.o.points[points_value].x, self.o.points[points_value].y);
                                }
                            });
                        }
                    });
                ...
                $.each(c, function(k, v){
                    c[k].fillStyle = territories_fill_style[k];
                    c[k].clearRect(0, 0, self.extended_canvas_width, self.extended_canvas_height);
                    c[k].beginPath();
                });
            }
            ...
            if(t === 'population'){
                $.each(self.territory_outlines, function(k, v){
                    var which_color = 0;
                    if(t === 'population'){
                        $.each(range, function(k2, v2){
                            if(v.count >= v2[0] && v.count <= v2[1]){
                                which_color = k2;
                                return false;
                            }
                        ...
                        which_color = Math.floor(Math.random()*6);
                    }
                    ...
                    $.each(v.outline, function(points_index, points_value){
                        // first is moveto
                        if(points_index === 0){
                            c[which_color].moveTo(self.o.points[points_value].x, self.o.points[points_value].y);
                            // all others are lineto
                        } else {
                            c[which_color].lineTo(self.o.points[points_value].x, self.o.points[points_value].y);
                        }
                    });
                });
                ...
                if(self.out_territories){
                    c[0].fillStyle = out_fill_style;
                    c[0].clearRect(0, 0, self.extended_canvas_width, self.extended_canvas_height);
                    c[0].beginPath();

                    var a = self.out_territories.slice(0);
                    $.each(self.territory_outlines, function(k, v){
                        if($.inArray(k, a) > -1){
                            // go through each point
                            $.each(v.outline, function(points_index, points_value){
        ...
        redrawSegments : function(obj){
            ...
            if(self.building_segment){
                ...
                // mark as different if it intersects another line
                // FIX - maybe not needed
                /*
                 if(self.lineIntersectsLine()){
                 c2.strokeStyle = self.no_draw_segment_stroke_color;
                 c2.beginPath();
                 c2.clearRect(0, 0, self.extended_canvas_width, self.extended_canvas_height);
                 c2.moveTo(self.o.points[self.drawing_from_point].x, self.o.points[self.drawing_from_point].y);

                 if(spring_to_point != null){
                 c2.lineTo(self.o.points[spring_to_point].x, self.o.points[spring_to_point].y);
                 } else {
                 c2.lineTo(self.x - parseInt($(self.canvases).css('left')), self.y - parseInt($(self.canvases).css('top')));
                 }
                ...
                // if(spring_to_point != null){
                // 	c.lineTo(self.o.points[spring_to_point].x, self.o.points[spring_to_point].y);
                // } else {
        ...
        totalAddressesInTerritory : function(points, which_side){
            var self = this,
                c = 0;

            if(which_side){
                self.highlight_overlaps[which_side] = [];
            }
            ...
            if(points){
                $.each(self.geocodes, function(geocodes_key, geocodes_value){
                    if(self.isInside(points, geocodes_value[0], geocodes_value[1])){
                        c += geocodes_value[2];
                        if(which_side){
                            self.highlight_overlaps[which_side].push(geocodes_key);
                        }
                    }
                });
            }
        ...
        setupCanvases : function(){
            ...
            // set extended canvas
            self.extended_canvas_width = (self.o.preferences.canvas_width * self.extended_canvas_size * 2) + self.o.preferences.canvas_width;
            self.extended_canvas_height = (self.o.preferences.canvas_height * self.extended_canvas_size * 2) + self.o.preferences.canvas_height;
            ...
            $(self.canvas_wrapper).css({'width':self.o.preferences.canvas_width+'px','height':self.o.preferences.canvas_height+'px'}).append('<div id="'+self.map_canvas+'" style="width: '+self.o.preferences.canvas_width+'px; height: '+self.o.preferences.canvas_height+'px;"></div>');
        ...
        loadPreferences : function(){
            ...
            //this.message(loaded ? null : 'Preferences could not be loaded:<br />');
